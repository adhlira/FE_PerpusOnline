...
const Detail = () => {
  ...
  const { id } = useParams();
  const EndPoint = `http://localhost:3000/pengembalian/${id}`;
        ...
        <div className="flex min-h-screen">
          ...
          <div className="flex-1 md:ml-5 ml-3 mt-5">
            <div className="flex flex-row justify-between">
              <p className="md:text-3xl text-2xl font-semibold">Detail Pengembalian</p>
            ...
            <table className="table table-auto min-w-full text-center border mt-8">
              <thead>
                <tr>
                  ...
                  <th className="md:w-2/12 py-2 px-4 block md:table-cell md:border md:text-center text-left">Denda</th>
                </tr>
              </thead>
              <tbody className="border">
                <tr className="border-b md:border-none block md:table-row">
                  <td className="block border  md:table-cell">{pengembalian.Petugas?.nama}</td>
                  <td className="block border  md:table-cell">{pengembalian.Anggota?.nama}</td>
                  <td className="block border  md:table-cell">{pengembalian.Anggota?.nis}</td>
                  <td className="block border  md:table-cell">{pengembalian.Detail_Pengembalian?.[0]?.Buku?.judul}</td>
                  <td className="block border  md:table-cell">{pengembalian.Detail_Pengembalian?.[0]?.Buku?.isbn}</td>
                  <td className="block border  md:table-cell">{formatDate(pengembalian.Peminjaman?.tanggal_pinjam)}</td>
                  <td className="block border  md:table-cell">{formatDate(pengembalian.Peminjaman?.tanggal_kembali)}</td>
                  <td className="block border  md:table-cell">{formatDate(pengembalian.tanggal_pengembalian)}</td>
                  <td className="block border  md:table-cell">Rp. {pengembalian.denda}</td>
                </tr>
              </tbody>
            </table>
          </div>

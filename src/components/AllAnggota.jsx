...
const Anggota = () => {
  ...
  const EndPoint = "http://localhost:3000/anggota";
  ...
  const currentItems = anggota.slice(indexOfFirstItem, indexOfLastItem);
        ...
        <div className="flex min-h-screen">
          ...
          <div className="flex-1 md:ml-5 ml-2 mt-5">
            <div className="flex flex-row justify-between">
              <p className="text-3xl font-semibold">Data Anggota</p>
              <Link to={"tambah"}>
                <button className="rounded-lg p-2 border bg-button1 text-putih hover:bg-button1hover md:mr-2">Tambah Data</button>
              </Link>
            </div>
              ...
              <table className="table table-auto min-w-full border mt-8">
                <thead>
                  <tr>
                    ...
                    <th className="w-2/12 py-2 px-4 block md:table-cell md:border md:text-center text-left">NIS</th>
                    <th className="w-3/12 py-2 px-4 block md:table-cell md:border md:text-center text-left">Nama</th>
                    <th className="md:w-2/12 py-2 px-4 block md:table-cell md:border md:text-center text-left">Jenis Kelamin</th>
                    <th className="w-2/12 py-2 px-4 block md:table-cell md:border md:text-center text-left">Telp</th>
                ...
                <tbody className="border text-center">
                  {currentItems?.map((item, index) => (
                    <tr className="border-b md:border-none block md:table-row" key={index}>
                      ...
                      <td className="block border md:table-cell">{item.nis}</td>
                      <td className="block border md:table-cell">{item.nama}</td>
                      <td className="block border md:table-cell">{item.jenis_kelamin}</td>
                      <td className="block border md:table-cell">{item.telp}</td>
                      <td className="block border md:table-cell">
                        <Link to={`edit/${item.id}`}>
                          <button className="rounded-lg p-2 bg-kuning text-putih hover:bg-kuninghover border ">Edit</button>
                        </Link>
                        <button className="rounded-lg p-2 bg-merah text-putih hover:bg-merahhover border  ml-2" onClick={() => deleteData(item.id)}>
                          Hapus
                        </button>
                      </td>
                    </tr>
            ...
            <div className="text-center mt-10">
              <Pagination totalItems={anggota.length} itemsPerPage={itemsPerPage} currentPage={currentPage} onPageChange={handlePageChange} />
            </div>
          </div>
        </div>
      </div>

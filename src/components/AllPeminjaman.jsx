...
const Peminjaman = () => {
  ...
  const EndPoint = "http://localhost:3000/peminjaman";
  ...
  const deleteData = async (id) => {
    ...
    if (cek == true) {
      try {
        ...
        setPinjam(pinjam.filter((item) => item.id !== id));
  ...
  const currentItems = pinjam.slice(indexOfFirstItem, indexOfLastItem);
        ...
        <div className="flex min-h-screen">
          ...
          <div className="flex-1 md:ml-5 ml-3 mt-5">
            <div className="flex flex-row justify-between">
              <p className="md:text-3xl text-2xl font-semibold">Data Peminjaman</p>
              <Link to={"tambah"}>
                <button className="rounded-lg p-2 bg-button1 hover:bg-button1hover text-putih border md:mr-2">Tambah Data</button>
              </Link>
            </div>
              ...
              <table className="table table-auto min-w-full border mt-8">
                <thead>
                  <tr>
                    ...
                    <th className="md:w-2/12 py-2 px-4 block md:table-cell md:border md:text-center text-left">Nama Anggota</th>
                ...
                <tbody className="border text-center">
                  {currentItems?.map((item, index) => (
                    <tr className="border-b md:border-none block md:table-row" key={index}>
                      ...
                      <td className="block border  md:table-cell">{formatDate(item.tanggal_pinjam)}</td>
                      <td className="block border  md:table-cell">{formatDate(item.tanggal_kembali)}</td>
                      <td className="block border  md:table-cell">
                        ...
                        <button className="rounded-lg p-2 bg-merah hover:bg-merahhover text-putih border ml-2" onClick={() => deleteData(item.id)}>
                          Hapus
                        </button>
                      </td>
                    </tr>
            ...
            <div className="text-center mt-10">
              <Pagination totalItems={pinjam.length} itemsPerPage={itemsPerPage} currentPage={currentPage} onPageChange={handlePageChange} />
            </div>
          </div>
        </div>
      </div>
